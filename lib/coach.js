...
const { getPlanStartMonday, getTrainingWeek, getWeekSchedule, summarizeZoneDistribution, classifyByHR } = require('./planUtils');
const activityTracker = require('./activityTracker');

// Fenêtre de recherche : ne traite que les activités plus récentes que ça.
// Volontairement courte (pas de mémoire persistante des activités déjà
// envoyées, voir activityTracker.js) : Jérôme déclenche l'appel lui-même
// juste après sa séance la plupart du temps, et le cron quotidien ne doit
// pas retraiter une activité déjà rapportée manuellement quelques heures
// plus tôt — d'où les doublons vus avec l'ancienne fenêtre de 26h. En
// contrepartie, une activité non déclenchée manuellement et vieille de plus
// de LOOKBACK_HOURS au moment du passage du cron ne sera jamais rapportée
// (le filet de sécurité ne couvre que les dernières heures, pas la
// journée entière).
const LOOKBACK_HOURS = Number(process.env.ACTIVITY_LOOKBACK_HOURS || 6);
...
module.exports = async (req, res) => {
  try {
    ...
    for (const activity of newActivities) {
      ...
      const coachAdvice = await coach.analyzeActivity(activityData, sleep, splits, trainingStatus, dynamics, weekZoneDistribution, weekSessions, weekVolume);
