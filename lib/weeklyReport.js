...
const { formatPace, parseGarminDate, isRunningActivity, isSwimmingActivity, summarizeWeight, summarizeSplits, summarizeTrainingStatus, summarizeRunningDynamics, summarizeRacePredictions } = require('./garminUtils');
...
module.exports = async (req, res) => {
  try {
    ...
    // Anti-rebond : évite de renvoyer le même bilan plusieurs fois en
    // quelques heures (déclenchements manuels rapprochés, tests) — voir le
    // commentaire dans activityTracker.js sur pourquoi ce n'est qu'un
    // anti-rebond court, pas un blocage "une fois par semaine".
    if (!activityTracker.isConfigured()) {
      console.log('⚠️ Pas de store Redis configuré (KV_REST_API_URL/KV_REST_API_TOKEN) : protection anti-doublon inactive, chaque déclenchement renverra le bilan.');
    } else if (await activityTracker.wasWeeklyReportSentRecently(currentWeek)) {
      console.log(`Bilan de la semaine ${currentWeek} déjà envoyé récemment, déclenchement ignoré`);
      return res.status(200).json({ message: 'Bilan déjà envoyé récemment pour cette semaine' });
    }
    ...
    for (const a of activities || []) {
      ...
      // Toujours récupérer les splits, sans essayer de deviner via un seuil
      // sur la FC moyenne : un vrai 5x1000m seuil (6/8 relevé chez Jérôme à
      // 164 bpm de moyenne) peut retomber sous n'importe quel seuil choisi
      // arbitrairement selon le ratio effort/récup, donc aucun seuil sur
      // cette moyenne ne peut fiablement repérer "cette séance a besoin des
      // segments" — c'est justement la moyenne qui pose problème.
      try {
        run.splits = summarizeSplits(await garmin.getActivitySplits(a.activityId));
      } catch (error) {
        console.log('Pas de détail par segment disponible pour cette activité (historique):', error.message);
      }

      runsSinceStart.push(run);
    }
    runsSinceStart.sort((a, b) => a.date - b.date);
    ...
    // La séance Récupération du plan peut être un footing OU une natation —
    // Garmin voit les deux, donc on compte aussi les séances de natation
    // pour ne pas sous-compter une semaine où elle a été faite à la piscine.
    const swimsSinceStart = (activities || [])
      .filter((a) => isSwimmingActivity(a.activityType?.typeKey))
      .map((a) => {
        const date = parseGarminDate(a.startTimeGMT);
        return { week: getTrainingWeek(date), date };
      })
      .filter((a) => a.date.getTime() >= planStart.getTime());

    const swimsByWeek = new Map();
    for (const swim of swimsSinceStart) {
      if (!swimsByWeek.has(swim.week)) swimsByWeek.set(swim.week, []);
      swimsByWeek.get(swim.week).push(swim);
    }

    // Réalisé vs prévu, semaine par semaine, jusqu'à la semaine en cours.
    // Le plan compte 3 séances "courtes" (Qualité #1, Qualité #2,
    // Récupération) séparément de la sortie longue (1) — 3+1 = les 4
    // séances du plan. On distingue donc les deux plutôt que de tout
    // regrouper dans un seul total (ce qui affichait un ratio faux, ex.
    // "5/4", sur une semaine par ailleurs conforme). La sortie longue est
    // présumée être la plus longue course de la semaine (même heuristique
    // que weeklyStats.longRun) ; une natation cette semaine-là compte comme
    // la séance Récupération faite.
    ...
    for (let w = 1; w <= currentWeek; w += 1) {
      ...
      const hasSwim = (swimsByWeek.get(w) || []).length > 0;
      // Utilise les splits déjà récupérés ci-dessus pour les séances
      // probablement structurées (avgHR > 165), null pour les autres — dans
      // ce cas summarizeZoneDistribution retombe sur la moyenne de
      // l'activité, ce qui reste fiable pour une séance easy/endurance à
      // effort homogène.
      const zoneDistribution = summarizeZoneDistribution(
        weekRuns.map((r) => ({ distanceKm: r.distanceKm, avgHR: r.avgHR, splits: r.splits }))
      );
      weeklyProgress.push({
        week: w,
        actualKm: Math.round(actualKm * 10) / 10,
        plannedKm: weekSchedule ? weekSchedule.volumeKm : null,
        shortSessionsCount: (weekRuns.length > 0 ? weekRuns.length - 1 : 0) + (hasSwim ? 1 : 0),
    ...
    for (const r of thisWeekRuns) {
      // r.splits est déjà rempli si avgHR > 165 (voir la première passe
      // ci-dessus) — on ne refait l'appel Garmin que si ce n'est pas le cas,
      // pour ne pas fetcher deux fois les mêmes segments.
      let splits = r.splits;
      if (!splits) {
        try {
    ...
    const longRun = longRunSource
      ...
      : null;
    const thisWeekSwims = swimsByWeek.get(currentWeek) || [];
    const activeDaysCount = new Set([...thisWeekRuns, ...thisWeekSwims].map((a) => a.date.toDateString())).size;
    ...
    try {
      racePredictions = summarizeRacePredictions(await garmin.getRacePredictions());
    } catch (error) {
      console.log('Pas de prédiction de course disponible:', error.message);
    }

    const coach = new CoachMarathon();
    const report = await coach.generateWeeklyReport(thisWeekActivities, weeklyProgress, weight, trainingStatus, weeklyStats, racePredictions);
    ...
    await emailSender.sendWeeklyReport(
      ...
      report,
      thisWeekActivities,
      thisWeekEntry ? thisWeekEntry.actualKm : null,
      thisWeekEntry ? thisWeekEntry.plannedKm : null,
      weeklyStats,
      trainingStatus,
      weeklyProgress,
      racePredictions
    ...
    await activityTracker.markWeeklyReportSent(currentWeek);
