...
const { formatPace, parseGarminDate, isRunningActivity, summarizeWeight, summarizeSplits, summarizeTrainingStatus } = require('./garminUtils');
const { getTrainingWeek, getPlanStartMonday } = require('./planUtils');
...
module.exports = async (req, res) => {
  try {
    ...
    for (const r of runsByWeek.get(currentWeek) || []) {
      let splits = null;
      try {
    ...
    const report = await coach.generateWeeklyReport(thisWeekActivities, weeklyProgress, weight, trainingStatus);
