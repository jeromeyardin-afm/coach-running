...
module.exports = async (req, res) => {
  try {
    ...
    for (const a of activities || []) {
      ...
      // Une séance easy/endurance a un effort homogène : la moyenne FC de
      // l'activité entière classe la bonne zone. Une séance qualité mêle
      // répétitions dures et récupération, et sa moyenne peut retomber dans
      // une zone plus facile que l'effort réel (même biais déjà corrigé pour
      // le rapport quotidien via les segments). On ne récupère donc les
      // splits que pour les séances dont la moyenne dépasse déjà le seuil
      // "Allure marathon" — probablement une séance structurée — plutôt que
      // pour tout l'historique, pour limiter les appels Garmin.
      if (avgHR > 165) {
        try {
