...
module.exports = async (req, res) => {
  try {
    ...
    for (let w = 1; w <= currentWeek; w += 1) {
      ...
      // Zone au niveau de l'activité entière (pas du segment) pour les
      // semaines passées, faute de récupérer les splits de chaque course de
      // tout l'historique — coûteux en appels Garmin pour un gain de
      // précision marginal sur des semaines déjà closes. Seule la semaine en
      // cours (thisWeekActivities plus bas) bénéficie du détail par segment.
      const zoneDistribution = summarizeZoneDistribution(
        weekRuns.map((r) => ({ distanceKm: r.distanceKm, avgHR: r.avgHR, splits: null }))
      );
      weeklyProgress.push({
        week: w,
