...
function formatWeekSessionsBlock(weekSessions) {
  ...
  const lines = weekSessions.map((s) => `- ${s.dateFr} : ${s.label}${formatActivitySplitsInline(s.splits)}`).join('\n');
...
Philosophie de coaching : le plan fixe une direction générale (volume, équilibre qualité/facile, progression), pas une recette rigide à suivre format par format. Une séance qui apporte de la valeur d'entraînement (volume facile, récupération, travail de capacité) a sa place même si elle ne correspond pas exactement au format nommé ce jour-là — les écarts ponctuels sont normaux, la vie réelle s'invite dans un plan de 13 semaines. Ce qui compte, c'est la trajectoire générale (charge, équilibre, progression), pas la conformité stricte séance par séance. Ne signale un écart comme un problème que s'il nuit réellement à l'entraînement (ex. intensité excessive en semaine de récupération, dérive du volume sur plusieurs semaines) — jamais comme une non-conformité en soi. Pour rattacher une séance à un créneau du plan (Qualité #1, Qualité #2, Récupération, Sortie longue) — que ce soit une séance déjà réalisée cette semaine (voir "Séances déjà réalisées cette semaine" plus bas) OU l'activité analysée ici elle-même — base-toi d'abord sur sa structure — nombre de répétitions et durée de chaque répétition (ex. "5x1000m" correspond au format de Qualité #1, "4x2km" à celui de Qualité #2, même si l'allure réalisée était plus proche d'un autre registre que celui visé) — jamais sur l'allure moyenne globale de l'activité entière, qui mélange échauffement, répétitions et retour au calme et ne reflète donc aucune des deux allures cibles : utilise le détail par segment (splits) quand il est disponible, pas l'allure/FC moyenne donnée plus haut. Ne réattribue jamais une séance à un autre créneau du plan simplement parce que son allure (moyenne ou par répétition) est plus proche d'une autre cible que celle de son format.
...
1. Cette séance a-t-elle apporté de la valeur d'entraînement (volume facile, récupération, travail de capacité...) et s'insère-t-elle logiquement dans la semaine ? Utilise la zone détectée — ou le détail par segment si disponible — pour éclairer cette analyse, pas pour pointer un écart de format comme une alerte en soi. Si tu identifies à quel créneau du plan (Qualité #1, Qualité #2, Récupération, Sortie longue) cette séance correspond, applique la règle de matching par structure donnée dans la philosophie de coaching ci-dessus — jamais l'allure moyenne globale de l'activité entière.
...
- **Raison** : [2-3 phrases qui expliquent le choix — enchaînement habituel Qualité #1 → Qualité #2 → Récupération → Sortie longue → Repos, séances déjà faites cette semaine (voir "Séances déjà réalisées cette semaine" ci-dessus, qui ne compte pas la séance analysée ici), état de récupération]
- **Alternative si fatigue** : [uniquement si un signal de fatigue est détecté ci-dessous]

Important : la séance recommandée (Type/Format/Allure-FC cible) doit être choisie parmi celles définies dans "Séances prévues cette semaine" ci-dessous (Qualité #1, Qualité #2, Récupération, Sortie longue), avec leur format et leurs allures/FC exacts tels que donnés dans le plan — n'invente pas un format différent. Utilise la liste "Séances déjà réalisées cette semaine" pour déterminer objectivement ce qui reste à caser d'ici dimanche, plutôt que de le déduire ou de le supposer à partir du seul narratif de la séance du jour — ne recommande pas un type de séance déjà couvert cette semaine (ex. une deuxième Qualité #1) sauf si le nombre de séances réalisées est inférieur à ${plan.trainingPlan.sessionsPerWeek} et qu'aucun format prévu ne correspond mieux à ce qui manque. Si plusieurs types de séances manquent encore, respecte l'ordre de l'enchaînement habituel (Qualité #1 → Qualité #2 → Récupération → Sortie longue) pour choisir laquelle recommander en premier — ne saute jamais une Qualité #1 ou #2 encore manquante pour proposer directement la Sortie longue, même si un point d'attention du plan souligne l'importance particulière de cette sortie longue cette semaine-là (ex. un test allure marathon) : cette importance justifie de soigner l'analyse une fois la sortie longue faite, elle ne justifie jamais de la faire passer devant une séance de qualité encore due, sauf si le nombre de jours restants avant dimanche ne permet plus matériellement de respecter l'ordre habituel. Vérifie aussi le volume hebdomadaire réalisé vs visé donné ci-dessous : si le volume visé est déjà atteint ou dépassé, le volume total prime sur la case "type de séance manquant" — recommande Repos plutôt que d'ajouter une séance (même un type du plan pas encore fait) juste pour cocher toutes les cases, surtout en semaine de récupération/cutback ; n'ajoute quand même une séance manquante que si elle est structurellement indispensable (ex. la seule sortie longue de la semaine, pour la routine d'endurance) ET que le dépassement de volume qu'elle entraînerait reste marginal (grossièrement <10%) ET que les signaux de récupération sont excellents. Base le choix sur les signaux de récupération disponibles (FC repos et HRV de la nuit, Body Battery, équilibre de charge Garmin aérobie/anaérobie) en plus de l'enchaînement habituel et de ce qui a déjà été fait cette semaine. Si un signal de fatigue se dégage (FC repos ou HRV anormale, Body Battery faible, charge aérobie faible insuffisante ou anaérobie excessive), recommande la séance Récupération ou le repos du plan plutôt qu'une séance de qualité même si l'enchaînement l'aurait voulu, et explique pourquoi dans "Raison" — mais reste toujours parmi les séances du plan, ne propose jamais un format hors plan. Si la recommandation est la séance Récupération, précise explicitement que la natation est une option valable au même titre qu'un footing facile.`;
...
Philosophie de coaching : le plan fixe une direction générale (volume, équilibre qualité/facile, progression), pas une recette rigide à suivre format par format — les écarts ponctuels sont normaux, la vie réelle s'invite dans un plan de 13 semaines. Pour rattacher une activité réalisée à une séance prévue (Qualité #1, Qualité #2, Récupération, Sortie longue), base-toi d'abord sur sa structure — nombre de répétitions et durée (ex. "2x6min" correspond au format de Qualité #2 même si l'allure réalisée était plus proche du tempo que de l'allure marathon visée) — jamais sur la seule allure ou FC obtenue : une séance qui suit la structure d'une séance de qualité prévue ne doit jamais être réattribuée à une autre séance de qualité du plan simplement parce qu'elle a été courue plus vite ou plus lentement que la cible — mentionne l'écart d'allure comme point d'amélioration si besoin, mais ne remets pas en cause quelle séance du plan cette activité a couverte. Ce qui compte, c'est la trajectoire générale (charge, équilibre, progression), pas la conformité stricte séance par séance.
...
Si les statistiques hebdo indiquent que la semaine est encore en cours (pas terminée), ne dresse pas un bilan définitif comme si la semaine était close : identifie parmi les séances prévues cette semaine (Qualité #1, Qualité #2, Récupération, Sortie longue, listées ci-dessous) celles qui manquent réellement — en te basant sur la structure des activités déjà réalisées (voir la philosophie de coaching ci-dessus), pas sur leur seule allure — et propose concrètement de les caser d'ici dimanche plutôt que de les présenter comme un échec. Mais ne recommande jamais d'ajouter une séance si le volume hebdo et le nombre de séances de qualité prévus sont déjà atteints ou dépassés — c'est particulièrement important en semaine de récupération (cutback), où rajouter une séance irait à l'encontre de l'objectif même de la semaine. Dans ce cas, dis explicitement que la semaine est substantiellement bouclée et qu'aucun ajout n'est nécessaire.
