...
Ne redis pas en chiffres ce que l'email affiche déjà visuellement : le volume de la semaine (barre), la répartition par zone de la semaine (barre + légende), la progression semaine par semaine — volume et zones pour chaque semaine du plan (section dédiée avec une carte par semaine), et l'équilibre de charge Garmin (barres). Commente-les seulement quand il y a quelque chose à expliquer (pourquoi ce déséquilibre, ce que ça implique) — jamais pour répéter la valeur déjà visible.

Style : sections 1 et 2 (résumé, progression) restent factuelles et concises — liste à puces courtes, 1 ligne par idée, pas de paragraphe narratif, pas de phrase d'intro/transition, pas de tournures du type "il est intéressant de noter que", pas de superlatifs. Pour la description des activités individuelles, une ligne courte par séance — chiffres + verdict. Exemple de niveau de détail attendu : "11/08 : 8,64km — 1,6km éch. facile + 5,3km tempo à 4:41 (FC174, conforme) + 1,7km retour au calme". Sections 3 et 4 (points d'amélioration, ajustements) peuvent en revanche développer davantage : 2-3 phrases par point pour expliquer le pourquoi et le comment, pas juste l'énoncer sèchement — c'est le conseil concret que Jérôme doit pouvoir appliquer.
...
2. **Progression globale vers l'objectif** : la tendance semaine par semaine est déjà visible en barres plus bas dans l'email — interprète-la (en avance, dans les clous, ou en retard sur le plan ?) sans la redécrire chiffre par chiffre. Le potentiel marathon estimé (${plan.physiology.semiMarathonRecord.predictedMarathonPotential}) reste-t-il crédible au vu de l'exécution jusqu'ici ?
...
5. Un mot d'encouragement (2-3 phrases)`;
