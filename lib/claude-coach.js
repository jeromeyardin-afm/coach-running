...
function formatWeeklyStatsBlock(stats) {
  ...
  if (stats.weekOverWeek) {
    ...
    const sign = (n) => (n >= 0 ? '+' : '');
    lines.push(`Vs semaine précédente : ${sign(w.deltaKm)}${w.deltaKm} km (${sign(w.deltaPercent)}${w.deltaPercent}%), ${sign(w.deltaSessions)}${w.deltaSessions} séance(s) courte(s)`);
  }
...
Donne un feedback court et actionnable :
...
- **Type** : [Qualité #1 seuil-VMA / Qualité #2 tempo-marathon / Easy court / Easy moyen / Sortie longue / Récup-Repos]
...
- **Raison** : [enchaînement habituel Qualité #1 → Easy court → Qualité #2 → Easy moyen → Sortie longue → Récup/Repos, séances déjà faites vs le plan cette semaine, état de récupération]
- **Alternative si fatigue** : [uniquement si un signal de fatigue est détecté ci-dessous]

Important : la séance recommandée (Type/Format/Allure-FC cible) doit être choisie parmi celles définies dans "Séances prévues cette semaine" ci-dessous (Qualité #1, Easy court, Qualité #2, Easy moyen, Sortie longue), avec leur format et leurs allures/FC exacts tels que donnés dans le plan — n'invente pas un format différent. Base le choix sur les signaux de récupération disponibles (FC repos et HRV de la nuit, Body Battery, équilibre de charge Garmin aérobie/anaérobie) en plus de l'enchaînement habituel et de ce qui a déjà été fait cette semaine. Si un signal de fatigue se dégage (FC repos ou HRV anormale, Body Battery faible, charge aérobie faible insuffisante ou anaérobie excessive), recommande la séance easy ou le repos du plan plutôt qu'une séance de qualité même si l'enchaînement l'aurait voulu, et explique pourquoi dans "Raison" — mais reste toujours parmi les séances du plan, ne propose jamais un format hors plan. Si la recommandation est un jour de récupération/repos, précise explicitement que la natation est une option valable.`;
  ...
  async generateWeeklyReport(activities, weeklyProgress = [], weight = null, trainingStatus = null, weeklyStats = null) {
    const progressTable = weeklyProgress
      .map((w) => {
        ...
        return `- Semaine ${w.week} (${w.focus || '—'}) : ${w.actualKm} km réalisés vs ${w.plannedKm ?? '?'} km prévus (${ecart}), ${w.shortSessionsCount}/${w.plannedSessionsPerWeek} séances courtes (qualité+easy), sortie longue ${w.longRunCompleted ? 'faite' : 'non faite'}`;
      })
      .join('\n');
...
Si les statistiques hebdo indiquent que la semaine est encore en cours (pas terminée), ne dresse pas un bilan définitif comme si la semaine était close : identifie parmi les séances prévues cette semaine (Qualité #1/#2, Easy court/moyen, Sortie longue, listées ci-dessous) celles qui manquent encore, et propose concrètement de les caser d'ici dimanche plutôt que de les présenter comme un échec.
