...
class CoachMarathon {
  async analyzeActivity(activity, sleep = null, splits = null) {
...
${activities.map(a => `- ${a.date} : ${a.type} ${a.distance}km à ${a.pace}/km (FC ${a.avgHR} bpm, zone ${classifyByHR(a.avgHR)}, cadence ${a.cadence || 'n/a'} pas/min)${formatActivitySplitsInline(a.splits)}`).join('\n')}
...
1. **Résumé de la semaine** : volume et charge d'entraînement, une ligne par activité (voir consigne de style ci-dessus), séances de qualité faites vs prévues (compare aux allures cibles de la séance qualité prévue cette semaine)
