...
class GarminClient {
  ...
  // Contrairement aux méthodes ci-dessous, celle-ci est officiellement
  // documentée par garmin-connect (GarminConnect.prototype.getActivity).
  // Le typage de la lib annonce IActivity (la forme plate de la liste),
  // mais l'endpoint réel (activity-service/activity/{id}, sans suffixe) est
  // en pratique connu pour renvoyer un objet plus riche imbriqué sous
  // summaryDTO (dynamique de course, training effect) — d'où la lecture
  // défensive à double forme dans summarizeRunningDynamics().
  ...
  // Officiellement documentée (GarminConnect.prototype.getUserProfile) —
  // sert ici uniquement à récupérer displayName, requis par l'URL du
  // prédicteur de course ci-dessous.
  async getUserProfile() {
    await this.login();
    return this.client.getUserProfile();
  }

  // Non documenté par garmin-connect. Chemin repris de python-garminconnect
  // (get_race_predictions) : /metrics-service/metrics/racepredictions/latest/{displayName}.
  // Forme de la réponse non vérifiée — voir /api/debug-race-predictions.
  async getRacePredictions() {
    await this.login();
    const profile = await this.client.getUserProfile();
    return this.client.get(
      `https://connectapi.garmin.com/metrics-service/metrics/racepredictions/latest/${profile.displayName}`
    );
  }
}
