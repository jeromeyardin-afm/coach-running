...
module.exports = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      planStartMonday: planStart.toISOString().slice(0, 10),
      currentWeek,
      totalActivitiesFetched: allActivities.length,
      weeklyBreakdown,
      excludedActivities: allActivities.filter((a) => !a.countedAsRunning && !a.beforePlanStart),
      runningActivitiesBeforePlanStart: allActivities.filter((a) => a.countedAsRunning && a.beforePlanStart),
    });
  } catch (error) {
