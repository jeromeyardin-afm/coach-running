...
// EMAIL_DESTINATAIRE peut contenir la même adresse listée deux fois par
// erreur (copier-coller malheureux dans les variables Vercel) — dans ce
// cas nodemailer émet un RCPT TO par occurrence et Gmail livre bien 2
// exemplaires du même email, ce qui ressemble à un doublon applicatif
// alors que c'est un seul envoi avec une liste de destinataires en double.
function dedupeRecipients(destinataire) {
  if (typeof destinataire !== 'string') return destinataire;
  const seen = new Set();
  const unique = destinataire
    .split(/[,;]/)
    .map((addr) => addr.trim())
    .filter((addr) => {
...
// pertinente pour juger une seule séance (Jérôme l'a fait remarquer) —
// seuls le statut et la charge aiguë/chronique (un ratio 7j/28j, pertinent
// au jour le jour) restent affichés côté quotidien.
function buildTrainingStatusVisual(trainingStatus, { includeMonthlyBalance = true } = {}) {
  ...
  if (!includeMonthlyBalance) return html;
  const rows = [
...
// Barre empilée + légende pour une répartition par zone, factorisée pour
// être réutilisée telle quelle par le rapport hebdo (semaine en cours) et
// le rapport quotidien (semaine en cours, vue depuis une seule séance).
function buildZoneDistributionVisual(zoneDistribution, label = 'Répartition par zone') {
  ...
  return `<div style="margin-bottom:14px;">
    ...
    <div style="display:flex;height:16px;border-radius:4px;overflow:hidden;margin-top:4px;">
      ${zoneDistribution.map((z) => `<div style="background:${ZONE_COLORS[z.zone] || '#95a5a6'};width:${z.percent}%;"></div>`).join('')}
    </div>
    <div style="font-size:11px;color:#666;margin-top:4px;">
      ${zoneDistribution.map((z) => `<span style="margin-right:10px;white-space:nowrap;"><span style="display:inline-block;width:8px;height:8px;background:${ZONE_COLORS[z.zone] || '#95a5a6'};border-radius:2px;margin-right:3px;"></span>${z.zone} ${z.percent}%</span>`).join('')}
    </div>
  </div>`;
}
...
function buildWeeklyVisualBlock(weekActualKm, weekPlannedKm, weeklyStats, trainingStatus) {
  ...
  html += buildZoneDistributionVisual(weeklyStats?.zoneDistribution);
...
class EmailSender {
  ...
  async sendCoachReport(destinataire, subject, coachAdvice, activityData, trainingStatus = null, weekZoneDistribution = null) {
    const marked = await getMarked();
    const detectedZone = classifyByHR(activityData.avgHR);
    const weekZoneVisual = buildZoneDistributionVisual(weekZoneDistribution, 'Répartition de la semaine en cours par zone');
    const statusVisual = buildTrainingStatusVisual(trainingStatus, { includeMonthlyBalance: false });
    const htmlContent = `
<!DOCTYPE html>
<html>
<head>
...
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      ${weekZoneVisual || statusVisual ? `<div class="activity">${weekZoneVisual}${statusVisual}</div>` : ''}
    ...
    const mailOptions = {
      ...
      to: dedupeRecipients(destinataire),
      subject: subject,
      html: htmlContent,
    };
